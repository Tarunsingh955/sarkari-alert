...
export default function JobCard({job}:{job:any}) {
  ...
  return (
    <a href={`/jobs/${job.slug}`} style={{textDecoration:'none',display:'block'}}>
      <div onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} style={{background:'#1e293b',borderRadius:12,padding:16,border:`1px solid ${hover?'#f59e0b60':urgent?'#ef444440':'#334155'}`,position:'relative',overflow:'hidden',cursor:'pointer',transition:'all 0.2s',transform:hover?'translateY(-2px)':'none',boxShadow:hover?'0 8px 24px rgba(0,0,0,0.3)':'none'}}>
        ...
        <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <h3 style={{fontSize:15,fontWeight:700,color:'#f1f5f9',marginBottom:4,lineHeight:1.3}}>{job.title}</h3>
            <p style={{fontSize:12,color:'#94a3b8',marginBottom:8}}>{job.department}</p>
            <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:10}}>
              ...
              {job.states?.name&&<span style={{background:'#33415522',color:'#94a3b8',fontSize:11,padding:'2px 8px',borderRadius:20}}>{job.states.name}</span>}
            </div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:8}}>
              <div style={{fontSize:12}}>
                <span style={{color:'#94a3b8'}}>Last Date: </span>
                <span style={{color:urgent?'#ef4444':'#f59e0b',fontWeight:600}}>{new Date(job.last_date).toLocaleDateString('en-IN')}</span>
                {days>0&&<span style={{color:urgent?'#ef4444':'#64748b',fontSize:11}}> ({days} din bache)</span>}
                {days<=0&&<span style={{color:'#ef4444',fontWeight:700}}> CLOSED</span>}
              </div>
              <span style={{background:'linear-gradient(135deg,#f59e0b,#d97706)',borderRadius:8,padding:'5px 14px',color:'#000',fontWeight:700,fontSize:11}}>{job.notify_text||'Apply →'}</span>
            </div>
          </div>
        </div>
      </div>
