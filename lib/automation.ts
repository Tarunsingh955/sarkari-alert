...
// Filler words that don't help identify WHICH recruitment a title is about
// — stripping these lets two differently-worded titles for the same
// recruitment (from two different source sites) still match.
const TITLE_FILLER_WORDS = new Set([
  'recruitment','apply','online','offline','post','posts','notification',
  'walk-in','walkin','for','the','and','a','an','of','in','on','to','at',
  'various','other','others','vacancy','vacancies','2024','2025','2026','2027',
])
...
function normalizeTitleWords(title: string): Set<string> {
  const words = (title || '')
    ...
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter(w => w.length > 2 && !TITLE_FILLER_WORDS.has(w))
...
function titlesLikelyMatch(a: Set<string>, b: Set<string>): boolean {
...
async function scrapeRSS(source: any): Promise<number> {
  ...
  // Pull recent titles (any source) to catch the SAME recruitment posted
  // with slightly different wording on a different aggregator site — the
  // exact-source_url check below only catches re-fetching the identical URL.
  const twoWeeksAgo = new Date(Date.now() - 14 * 86400000).toISOString()
  const { data: recentQueueItems } = await supabaseAdmin.from('automation_queue').select('title,type').gte('created_at', twoWeeksAgo)
  const { data: recentJobs } = await supabaseAdmin.from('jobs').select('title').gte('created_at', twoWeeksAgo)
  const recentTitleSets = [
    ...(recentQueueItems || []).map(r => ({ type: r.type, words: normalizeTitleWords(r.title) })),
    ...(recentJobs || []).map(r => ({ type: 'job', words: normalizeTitleWords(r.title) })),
  ]
  ...
  for (const item of (feed.items || []).slice(0, 15)) {
    ...
    const newTitleWords = normalizeTitleWords(title)
    const isDuplicateAcrossSources = recentTitleSets.some(r => r.type === itemType && titlesLikelyMatch(newTitleWords, r.words))
