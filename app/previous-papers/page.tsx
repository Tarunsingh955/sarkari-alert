...
export default async function PreviousPapersPage({ searchParams }: { searchParams: any }) {
  const { papers, total } = await getPapers(searchParams)
  return (
      ...
      <main style={{ maxWidth: 1100, margin: '0 auto', padding: 16 }}>
        ...
        <div style={{ background: '#1e293b', borderRadius: 12, padding: 16, marginBottom: 16, border: '1px solid #334155' }}>
          <form method="GET" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <input name="search" defaultValue={searchParams.search || ''} placeholder="🔍 Search papers..." style={{ width: '100%', padding: '10px 14px', background: '#0f172a', border: '1px solid #334155', borderRadius: 8, color: '#fff', fontSize: 13, outline: 'none', boxSizing: 'border-box' }} />
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: '#64748b', alignSelf: 'center' }}>Exam:</span>
              <a href="/previous-papers" style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: !searchParams.category ? '#f59e0b' : '#0f172a', border: `1px solid ${!searchParams.category ? '#f59e0b' : '#334155'}`, color: !searchParams.category ? '#000' : '#94a3b8', textDecoration: 'none' }}>Sab</a>
              {CATS.map(c => <a key={c} href={`/previous-papers?category=${c}`} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: searchParams.category === c ? '#f59e0b' : '#0f172a', border: `1px solid ${searchParams.category === c ? '#f59e0b' : '#334155'}`, color: searchParams.category === c ? '#000' : '#94a3b8', textDecoration: 'none' }}>{c}</a>)}
            </div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: '#64748b', alignSelf: 'center' }}>Year:</span>
              <a href="/previous-papers" style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: !searchParams.year ? '#8b5cf6' : '#0f172a', border: `1px solid ${!searchParams.year ? '#8b5cf6' : '#334155'}`, color: !searchParams.year ? '#fff' : '#94a3b8', textDecoration: 'none' }}>All Years</a>
              {YEARS.map(y => <a key={y} href={`/previous-papers?year=${y}`} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, background: Number(searchParams.year) === y ? '#8b5cf6' : '#0f172a', border: `1px solid ${Number(searchParams.year) === y ? '#8b5cf6' : '#334155'}`, color: Number(searchParams.year) === y ? '#fff' : '#94a3b8', textDecoration: 'none' }}>{y}</a>)}
            </div>
