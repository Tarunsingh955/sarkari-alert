  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 900, color: '#f1f5f9', margin: 0 }}>📋 Results ({results.length})</h1>
        <button onClick={() => setShowForm(!showForm)} style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', borderRadius: 8, padding: '10px 20px', color: '#000', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>{showForm ? 'Cancel' : '+ Add Result'}</button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: '#1e293b', borderRadius: 14, padding: 24, marginBottom: 20, border: '1px solid #334155' }}>
          <h2 style={{ color: '#f59e0b', fontSize: 15, marginBottom: 18 }}>New Result Add Karo</h2>
          ...
          <button type="submit" style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', borderRadius: 8, padding: '12px 32px', color: '#000', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>Add Result</button>
        </form>
      )}
      ...
      {loading ? <div style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>Loading...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {results.map((r: any) => (
            <div key={r.id} style={{ background: '#1e293b', borderRadius: 10, padding: '14px 16px', border: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ color: '#64748b', fontSize: 11, marginTop: 2 }}>{r.release_date || 'No date'}</div>
              </div>
          ...
          {!results.length && <div style={{ textAlign: 'center', padding: 32, color: '#64748b' }}>Koi result nahi. Upar se add karein.</div>}
        </div>
      )}
    </div>
  )
