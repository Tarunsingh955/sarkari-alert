...
export default function AdminAdmitCardsPage() {
  ...
  const blank = { title: '', release_date: '', exam_date: '', download_link: '', details: '' }
  ...
  async function fetchItems() {
    ...
    setItems(data.admitCards || [])
    setLoading(false)
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const res = await fetch('/api/admin/admit-cards', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    const data = await res.json()
    if (data.success) { showMsg('Admit Card add ho gaya!'); setShowForm(false); setForm(blank); fetchItems() }
    else showMsg('Error: ' + data.error, 'error')
  }

  async function deleteItem(id: string) {
    if (!confirm('Delete karein?')) return
    await fetch('/api/admin/admit-cards', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
    showMsg('Deleted!')
    fetchItems()
  }
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: '#1e293b', borderRadius: 14, padding: 24, marginBottom: 20, border: '1px solid #334155' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            ...
            <div><label style={lS}>Exam Date</label><input value={form.exam_date} onChange={e => setForm((f: any) => ({ ...f, exam_date: e.target.value }))} placeholder="15 August 2026" style={iS} /></div>
          </div>
          <label style={lS}>Download Link</label>
          ...
          {items.map((r: any) => (
            <div key={r.id} style={{ background: '#1e293b', borderRadius: 10, padding: '14px 16px', border: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <button onClick={() => deleteItem(r.id)} style={{ background: '#ef444420', border: '1px solid #ef444440', borderRadius: 6, padding: '4px 12px', color: '#ef4444', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>Delete</button>
              </div>
            </div>
          ))}
