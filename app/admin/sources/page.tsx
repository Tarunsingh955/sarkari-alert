  ...
  return (
    <div style={{padding:24}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        <h1 style={{fontSize:20,fontWeight:900,color:'#f1f5f9',margin:0}}>🤖 Automation Sources</h1>
        <button onClick={runAuto} disabled={running} style={{background:running?'#334155':'linear-gradient(135deg,#10b981,#059669)',border:'none',borderRadius:8,padding:'10px 20px',color:running?'#64748b':'#fff',fontWeight:700,fontSize:13,cursor:running?'not-allowed':'pointer'}}>{running?'Running...':'▶ Run Automation Now'}</button>
      </div>
      {/* Add Source Form */}
      <form onSubmit={addSource} style={{background:'#1e293b',borderRadius:14,padding:20,marginBottom:20,border:'1px solid #334155'}}>
        <h2 style={{color:'#f59e0b',fontSize:14,fontWeight:800,marginBottom:14}}>➕ Naya Source Add Karo</h2>
        ...
        <button type="submit" style={{background:'linear-gradient(135deg,#f59e0b,#d97706)',border:'none',borderRadius:8,padding:'10px 24px',color:'#000',fontWeight:700,fontSize:13,cursor:'pointer'}}>Add Source</button>
      </form>
      {/* Recommended Free Sources */}
      <div style={{background:'#1e293b',borderRadius:12,padding:18,marginBottom:20,border:'1px solid #334155'}}>
        <h3 style={{color:'#f59e0b',fontSize:13,fontWeight:700,marginBottom:12}}>💡 Recommended Free RSS Feeds</h3>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[['Employment News','https://www.employmentnews.gov.in/rss','general'],['UPSC','https://www.upsc.gov.in/rss','upsc'],['SSC','https://ssc.nic.in/rss','ssc'],['PIB India','https://www.pib.gov.in/rss/rss.aspx','general'],['Govt Jobs India','https://www.govtjobsindia.net/feed','general']].map(([n,u,c])=>(
            <div key={n} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 0',borderBottom:'1px solid #0f172a',gap:12,flexWrap:'wrap'}}>
              <div><div style={{color:'#f1f5f9',fontSize:13,fontWeight:600}}>{n}</div><div style={{color:'#64748b',fontSize:11}}>{u}</div></div>
      ...
      {loading?<div style={{textAlign:'center',color:'#64748b',padding:40}}>Loading...</div>:(
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {sources.map((s:any)=>(
            <div key={s.id} style={{background:'#1e293b',borderRadius:10,padding:'14px 16px',border:'1px solid #334155',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap'}}>
              <div style={{flex:1}}>
                <div style={{color:'#f1f5f9',fontSize:14,fontWeight:700,marginBottom:2}}>{s.name}</div>
                <div style={{color:'#64748b',fontSize:11,wordBreak:'break-all'}}>{s.url}</div>
                <div style={{display:'flex',gap:8,marginTop:6}}>
                  <span style={{background:'#f59e0b20',color:'#f59e0b',fontSize:10,padding:'1px 8px',borderRadius:20,fontWeight:700}}>{s.type?.toUpperCase()}</span>
                  <span style={{background:'#3b82f620',color:'#3b82f6',fontSize:10,padding:'1px 8px',borderRadius:20}}>{s.category}</span>
                  <span style={{fontSize:11,color:'#64748b'}}>Last check: {s.last_checked?new Date(s.last_checked).toLocaleDateString('en-IN'):'Never'}</span>
                </div>
              </div>
              <div style={{display:'flex',gap:8,flexShrink:0}}>
                <button onClick={()=>toggleSource(s.id,!s.is_active)} style={{background:s.is_active?'#10b98120':'#33415520',border:`1px solid ${s.is_active?'#10b98140':'#33415540'}`,borderRadius:6,padding:'5px 12px',color:s.is_active?'#10b981':'#64748b',fontSize:12,cursor:'pointer',fontWeight:600}}>{s.is_active?'Active':'Inactive'}</button>
          ...
          {!sources.length&&<div style={{textAlign:'center',padding:32,color:'#64748b'}}>Koi source nahi hai. Upar se add karein.</div>}
        </div>
      )}
    </div>
  )
