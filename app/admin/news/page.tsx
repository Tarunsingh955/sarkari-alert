  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 900, color: colors.textPrimary, margin: 0 }}>Manage News ({news.length})</h1>
        <button onClick={() => setShowForm(!showForm)} style={{ background: `linear-gradient(135deg,${colors.accent},${colors.accentDark})`, border: 'none', borderRadius: 8, padding: '10px 20px', color: '#000', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>{showForm ? 'Cancel' : '+ Add News'}</button>
      </div>
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: colors.cardBg, borderRadius: 14, padding: 24, marginBottom: 20, border: `1px solid ${colors.cardBorder}` }}>
          <h2 style={{ color: colors.accent, fontSize: 15, marginBottom: 18 }}>New News Add Karo</h2>
          ...
          <div style={{ display: 'flex', gap: 20, marginBottom: 14 }}>
            {[['is_urgent', 'Urgent News'], ['is_published', 'Publish Now']].map(([k, l]) => (
              <label key={k} style={{ display: 'flex', gap: 8, alignItems: 'center', color: colors.textSecondary, fontSize: 13, cursor: 'pointer' }}><input type="checkbox" checked={(form as any)[k]} onChange={e => setForm(f => ({ ...f, [k]: e.target.checked }))} /> {l}</label>
            ))}
          </div>
          <button type="submit" style={{ background: `linear-gradient(135deg,${colors.accent},${colors.accentDark})`, border: 'none', borderRadius: 8, padding: '12px 32px', color: '#000', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>Add News</button>
        </form>
      )}
      {loading ? <div style={{ textAlign: 'center', color: colors.textMuted, padding: 40 }}>Loading...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {news.map((n: any) => (
            <div key={n.id} style={{ background: colors.cardBg, borderRadius: 10, padding: '12px 16px', border: `1px solid ${colors.cardBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: colors.textPrimary, fontSize: 13, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{n.title}</div>
                <div style={{ color: colors.textMuted, fontSize: 11, marginTop: 2 }}>{n.category} • {new Date(n.created_at).toLocaleDateString('en-IN')}{n.is_urgent ? ' • 🔴 URGENT' : ''}</div>
              </div>
