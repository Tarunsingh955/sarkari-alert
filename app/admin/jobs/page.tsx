  ...
  return (
    <div style={{padding:24}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
        ...
        <button onClick={()=>{ if(showForm) cancelForm(); else setShowForm(true) }} style={{background:'linear-gradient(135deg,#f59e0b,#d97706)',border:'none',borderRadius:8,padding:'10px 20px',color:'#000',fontWeight:700,fontSize:13,cursor:'pointer'}}>{showForm?'Cancel':'+ Add New Job'}</button>
      </div>
      {showForm&&(
        <form onSubmit={handleSubmit} style={{background:'#1e293b',borderRadius:14,padding:24,marginBottom:20,border:'1px solid #334155'}}>
          <h2 style={{color:'#f59e0b',fontSize:15,marginBottom:18}}>{editingId?'Job Edit Karo':'New Job Add Karo'}</h2>
          ...
          <div style={{display:'flex',gap:20,marginBottom:16,flexWrap:'wrap'}}>
            {[['is_new','NEW Badge'],['is_hot','HOT Badge'],['is_published','Publish Now']].map(([k,l])=>(
              <label key={k} style={{display:'flex',gap:8,alignItems:'center',color:'#94a3b8',fontSize:13,cursor:'pointer'}}><input type="checkbox" checked={(form as any)[k]} onChange={e=>setForm((f:any)=>({...f,[k]:e.target.checked}))}/> {l}</label>
            ))}
          </div>
          <button type="submit" style={{background:'linear-gradient(135deg,#f59e0b,#d97706)',border:'none',borderRadius:8,padding:'12px 32px',color:'#000',fontWeight:800,fontSize:14,cursor:'pointer'}}>{editingId?'Update Job':'Add Job & Send Notifications'}</button>
        </form>
      )}
