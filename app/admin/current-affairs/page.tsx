  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div><h1 style={{ fontSize: 20, fontWeight: 900, color: colors.textPrimary, margin: 0 }}>Current Affairs MCQ ({items.length})</h1></div>
        <button onClick={() => setShowForm(!showForm)} style={{ background: `linear-gradient(135deg,${colors.accent},${colors.accentDark})`, border: 'none', borderRadius: 8, padding: '10px 20px', color: '#000', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>{showForm ? 'Cancel' : '+ Add Question'}</button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: colors.cardBg, borderRadius: 14, padding: 24, marginBottom: 20, border: `1px solid ${colors.cardBorder}` }}>
          <h2 style={{ color: colors.accent, fontSize: 15, marginBottom: 16 }}>New MCQ Question</h2>
          ...
          <button type="submit" style={{ background: `linear-gradient(135deg,${colors.accent},${colors.accentDark})`, border: 'none', borderRadius: 8, padding: '12px 32px', color: '#000', fontWeight: 800, fontSize: 14, cursor: 'pointer' }}>Add Question</button>
        </form>
      )}
      ...
      {loading ? <div style={{ textAlign: 'center', color: colors.textMuted, padding: 40 }}>Loading...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {items.map((item: any) => (
            <div key={item.id} style={{ background: colors.cardBg, borderRadius: 10, padding: '14px 16px', border: `1px solid ${colors.cardBorder}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ flex: 1 }}>
                <p style={{ color: colors.textPrimary, fontSize: 14, fontWeight: 600, margin: 0 }}>{item.question}</p>
                <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ color: colors.textMuted, fontSize: 11 }}>{item.month} {item.year}</span>
                </div>
              </div>
          ...
          {!items.length && <div style={{ textAlign: 'center', color: colors.textMuted, padding: 40 }}>Abhi koi question nahi hai — "+ Add Question" se add karo!</div>}
        </div>
      )}
    </div>
  )
