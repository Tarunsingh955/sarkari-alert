...
export async function GET(req: NextRequest) {
  ...
  const { data, count } = await supabaseAdmin.from('results').select('*', { count: 'exact' }).order('created_at', { ascending: false })
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin.from('results').insert({ ...body, slug }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    await logAdminAction(admin.id, 'CREATE_RESULT', { result_id: data.id })
...
export async function DELETE(req: NextRequest) {
  ...
  await supabaseAdmin.from('results').delete().eq('id', id)
  await logAdminAction(admin.id, 'DELETE_RESULT', { result_id: id })
