  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 900, color: '#f1f5f9', margin: 0 }}>SEO Settings</h1>
        <p style={{ color: '#64748b', fontSize: 12, marginTop: 4 }}>Site ka SEO aur integrations configure karo</p>
      </div>
      ...
      <form onSubmit={handleSave}>
        ...
        <div style={{ background: '#1e293b', borderRadius: 14, padding: 20, border: '1px solid #334155', marginBottom: 16 }}>
          <h3 style={{ color: '#f59e0b', fontSize: 14, marginBottom: 16 }}>🔍 Basic SEO</h3>
          ...
          <input value={settings.site_keywords} onChange={e => setSettings(s => ({ ...s, site_keywords: e.target.value }))} placeholder="sarkari job, government job, sarkari result..." style={iS} />
        </div>
        {/* Analytics */}
        <div style={{ background: '#1e293b', borderRadius: 14, padding: 20, border: '1px solid #334155', marginBottom: 16 }}>
          <h3 style={{ color: '#f59e0b', fontSize: 14, marginBottom: 16 }}>📊 Analytics & Verification</h3>
          <label style={lS}>Google Analytics ID (G-XXXXXXXXXX)</label>
          <input value={settings.ga_id} onChange={e => setSettings(s => ({ ...s, ga_id: e.target.value }))} placeholder="G-XXXXXXXXXX" style={iS} />
          <label style={lS}>Google Site Verification Code</label>
        ...
        {/* Monetization */}
        <div style={{ background: '#1e293b', borderRadius: 14, padding: 20, border: '1px solid #334155', marginBottom: 16 }}>
          <h3 style={{ color: '#f59e0b', fontSize: 14, marginBottom: 16 }}>💰 Monetization</h3>
          <label style={lS}>Google AdSense ID (ca-pub-XXXXXXXXXX)</label>
        ...
        <button type="submit" disabled={saving} style={{ background: saving ? '#334155' : 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', borderRadius: 8, padding: '12px 32px', color: saving ? '#64748b' : '#000', fontWeight: 800, fontSize: 14, cursor: saving ? 'not-allowed' : 'pointer' }}>
